...
class HomeController extends Controller
{
    public function index(Request $request)
    {
            ...
            ->where('private', false)
            ->orderBy('webinars.id', 'desc')
            ->join('categories', 'categories.id', '=', 'webinars.category_id')
            ->where('categories.id', 632)
            ->paginate(4);

            $webinarTrending = Webinar::select('webinars.*')
            ->where('webinars.status', 'active')
            ->where('private', false)
            ->orderBy('webinars.id', 'desc')
            ->join('tags', 'tags.webinar_id', '=', 'webinars.id')
            ->where('tags.title', 'trending')
            ->paginate(4);
            if(!$webinarTrending){
                $webinar = $webinarAll;
            }else{
