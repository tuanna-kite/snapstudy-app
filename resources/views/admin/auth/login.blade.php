...
@section('content')
    ...
    <div class="p-4 m-3">
        ...
        <form method="POST" action="{{ getAdminPanelUrl() }}/login" class="needs-validation">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="form-group">
                <div class="d-block">
                    <label for="email">{{ trans('auth.email') }}</label>
                </div>
                <input id="email" type="text" value="{{ old('email') }}" class="form-control  @error('email')  is-invalid @enderror"
