    ...
    <section class="section">
        ...
        <div class="section-body card">
            <div class="row">
                <div class="col-12 col-md-4 pr-0">
                    <div class="card-body">
                        <form action="{{ getAdminPanelUrl() }}/financial/documents/store" method="post">
                            ...
                            @if (!empty($currencies) and count($currencies))
                                <div class="form-group">
                                    ...
                                    <select name="currency" class="js-ajax-currency form-control select2"
                                        data-placeholder="{{ trans('admin/main.currency') }}">
                                        <option value=""></option>
                                        @foreach ($currencies as $currencyItem)
                                            <option value="{{ $currencyItem->currency }}"
                                                {{ currency() == $currencyItem->currency ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                <label class="control-label">{{ trans('admin/main.amount') }}
                                    ({{ currencySign(getDefaultCurrency()) }})</label>
                ...
                @include('admin.financial.documents.historyRequest');
            </div>
        </div>
    </section>
@endsection
