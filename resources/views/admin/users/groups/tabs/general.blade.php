    ...
    <div class="row">
        <div class="col-12 col-md-8 col-lg-6">
            <form action="{{ getAdminPanelUrl() }}/users/groups/{{ !empty($group) ? $group->id.'/update' : 'store' }}" method="Post">
                ...
                <div class="form-group">
                    <label>{{ trans('admin/main.min_score') }}</label>
                    <input type="text" name="min_score"
                           class="form-control  @error('min_score') is-invalid @enderror"
                           value="{{ !empty($group) ? $group->min_score : old('min_score') }}"/>
                    @error('min_score')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <div class="form-group">
                    <label>{{ trans('admin/main.max_score') }}</label>
                    <input type="text" name="max_score"
                           class="form-control  @error('max_score') is-invalid @enderror"
                           value="{{ !empty($group) ? $group->max_score : old('max_score') }}"/>
                    @error('max_score')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
