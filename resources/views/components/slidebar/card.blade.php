...
<div class="rounded-lg mb-2">
    @if (is_array($menuItem['tab']))
        <button x-effect="console.log(openTab)"  @click="openTab ===  {{ $menuItem['href'] }}  ? openTab = null : openTab = {{ $menuItem['href'] }}"
            class="relative z-10 w-full">
            <div :class="{
                'bg-primary.light': openTab === {{ $menuItem['href'] }},
                'bg-transparent': openTab !==
                    {{ $menuItem['href'] }}
            }"
        ...
        <div x-show="openTab === {{ $menuItem['href'] }}"
            ...
            <div class="min-w-48 -mt-4 mb-4">
                @foreach ($menuItem['tab'] as $key => $value)
                    <x-slidebar.sub-card :title="$key" :href="$value" />
                @endforeach
            </div>
        </div>
    @endif
