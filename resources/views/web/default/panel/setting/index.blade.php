...
@section('content')
    ...
    <div class="create-webinar-footer d-flex align-items-center  mt-20 pt-15 border-top ">
        <div class="d-flex align-items-center ">
            @if(!empty($user) and empty($new_user))
                @if(!empty($currentStep) and $currentStep > 1)
                    <a href="/panel/setting/step/{{ ($currentStep - 1) }}" class="btn btn-sm btn-primary mr-2">{{ trans('webinars.previous') }}</a>
                @else
                    <a href="" class="btn btn-sm btn-primary disabled">{{ trans('webinars.previous') }}</a>
                @endif

                <button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-1" @if(!empty($currentStep) and (($user->isUser() and $currentStep == 8))) disabled @endif>{{ trans('webinars.next') }}</button>
            @endif
        </div>

        <div class="d-flex align-items-center">
            @if(empty($new_user) and empty($edit_new_user))
                <a href="/panel/setting/deleteAccount" class="delete-action btn btn-sm btn-danger ml-2" data-confirm="{{ trans('update.delete_account_modal_confirm_btn_text') }}" data-title="{{ trans('update.delete_account_modal_hint') }}">{{ trans('update.delete_account') }}</a>
            @endif

            <button type="button" id="saveData" class="btn btn-sm btn-primary ml-2">{{ trans('public.save') }}</button>
        </div>
    </div>
@endsection
